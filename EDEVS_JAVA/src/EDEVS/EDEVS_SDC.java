...
public class EDEVS_SDC extends JPanel {
	...
	private void Check() {
		...
		Check[0][2] = 1000;
		Check[1][2] = MySDC.velocity - MySDC2.velocity;
		Check[2][2] = MySDC.velocity - MySDC3.velocity;
		Check[3][2] = MySDC.velocity - MySDC4.velocity;
		Check[4][2] = MySDC.velocity - MySDC5.velocity;

		CTime[0] = 1000;
		for (int i = 1; i < 5; i++) {
			if (Check[i][2] != 0) {
				CTime[i] = Math.abs(Check[i][0] / Check[i][2]);
			} else
				CTime[i] = 1000;
		}
		...
		for (int j = 1; j < 5; j++) {
			...
			if (CTime[j] < 30 && Math.abs(Check[j][1]) == 150) {
				System.out.printf("CrushTime = %d ", CTime[j]);
				System.out.printf("y차이 = %d ", Math.abs(Check[j][1]));
				System.out.printf("%d번차 때문에 ", j + 1);
				System.out.print("차선변경 불가능\n");
			}
